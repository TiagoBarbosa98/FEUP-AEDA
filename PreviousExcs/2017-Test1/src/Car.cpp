...
bool Car::operator < (const Car &car) const{
	if(brand.compare(car.getBrand()) == 0)
		return price > car.getPrice();
	else
		return(brand.compare(car.getBrand()) < 0);
}
...
FuelCar::FuelCar(std::string brand, std::string model, float price, float tank,
		float l_100km): Car(brand, model), tank(tank), l_100km(l_100km) {
	setPrice(price);
}
...
ElectricCar::ElectricCar(std::string brand, std::string model, float price,
		float battery, float kWh_100km): Car(brand, model), battery(battery), kWh_100km(kWh_100km) {
	setPrice(price);
}
...
return (100 * getTank() / getL_100km()) + (100 * getBattery() / getKWh_100km());
}
